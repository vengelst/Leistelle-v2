/**
 * Uebersetzt Axis-IP-Kamera-Ereignisse in das externe Alarm-Ingestion-Format.
 */
import type {
...
export type AxisIpCameraAlarmAdapterService = {
  ingest: (input: AxisIpCameraAlarmIngestionRequest, requestId: string, providedSharedSecret?: string) => Promise<ExternalAlarmIngestionResult>;
};

type CreateAxisIpCameraAlarmAdapterInput = {
  externalAlarmIngestion: Pick<ExternalAlarmIngestionService, "ingest">;
};

export function createAxisIpCameraAlarmAdapter(
  input: CreateAxisIpCameraAlarmAdapterInput
): AxisIpCameraAlarmAdapterService {
  return {
    async ingest(payload, requestId, providedSharedSecret) {
      const normalized = normalizeAxisIpCameraAlarm(payload);
      return await input.externalAlarmIngestion.ingest(normalized, requestId, providedSharedSecret);
    }
  };
}

export function normalizeAxisIpCameraAlarm(payload: AxisIpCameraAlarmIngestionRequest): ExternalAlarmIngestionRequest {
  const primarySerialNumber = payload.cameraSerialNumber?.trim();
  const primaryNetworkAddress = payload.cameraIp?.trim();
  const eventType = mapAxisCameraEvent(payload.eventCode, payload.eventType, payload.analyticsName);
  const severity = mapAxisCameraSeverity(payload.severity);
...
function mapAxisCameraEvent(eventCode: string, eventType?: string, analyticsName?: string): string {
  const candidates = [eventCode, eventType, analyticsName]
    .map((value) => value?.trim().toLowerCase())
    .filter((value): value is string => Boolean(value && value.length > 0));

  for (const candidate of candidates) {
    const mapped = axisCameraEventCodeMap[candidate];
    if (mapped) {
      return mapped;
    }
  }
...
function mapAxisCameraSeverity(rawSeverity: string | undefined): string | undefined {
  if (!rawSeverity?.trim()) {
    return undefined;
  }
...
function buildTitle(payload: AxisIpCameraAlarmIngestionRequest, eventType: string): string {
  ...
  return `Axis Camera ${location} | ${formatVendorEventLabel(eventType)}`;
}

function buildDescription(payload: AxisIpCameraAlarmIngestionRequest): string | undefined {
...
function buildRawPayload(payload: AxisIpCameraAlarmIngestionRequest, eventType: string): Record<string, unknown> {
  return {
    adapter: "axis-ip-camera",
